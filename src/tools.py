...
def get_only_in_img_mask(pts, H, W):
    """pts should be 3 x N"""
    return (
        (pts[2] > 0) & (pts[0] > 1) & (pts[0] < W - 1) & (pts[1] > 1) & (pts[1] < H - 1)
    )
...
def plot_nusc_map(rec, nusc_maps, nusc, scene2map, dx, bx):
    egopose = nusc.get(
        "ego_pose", nusc.get("sample_data", rec["data"]["LIDAR_TOP"])["ego_pose_token"]
    )
    ...
    center = np.array(
        [egopose["translation"][0], egopose["translation"][1], np.cos(rot), np.sin(rot)]
    )
